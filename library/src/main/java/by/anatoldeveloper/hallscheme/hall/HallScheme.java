...
public class HallScheme {
    ...
    private int width, height, totals=0;
    private Seat[][] seats;
    ...
    List<Seat> listofSelectedSeats=new ArrayList<Seat>();
...
//        Log.d("","drawTextCentred clickScheme findZoneClick(point) values "+(findZoneClick(point)));
...
//            Log.d("","drawTextCentred clickScheme row values "+(row)+" seat value "+(seat)+" p value "+(p));
//            Log.d("","HallScheme canSeatPress(p, row, seat) values "+(canSeatPress(p, row, seat)));

            if (canSeatPress(p, row, seat)) {
//                Log.d("","drawTextCentred clickScheme clickScheme(row, seat) values ");
...
//        Log.d("","Test Log clickScheme pressedSeat value "+(pressedSeat));
//        Log.d("","Test Log clickScheme updateSelectedSeatCount(pressedSeat) value "+(updateSelectedSeatCount(pressedSeat)));

        if (updateSelectedSeatCount(pressedSeat)) {

            notifySeatListener(pressedSeat);
//            Log.d("","Test Log clickScheme pressedSeat.status().pressSeat() value "+(pressedSeat.status().pressSeat()));

           pressedSeat.setStatus(pressedSeat.status().pressSeat());

           pressedSeat.price();
           totals=totals+pressedSeat.price();
            pressedSeat.setTotal(totals);
            Log.d("","Test Log clickScheme pressedSeat.marker() value "+(pressedSeat.marker())
                    +"pressedSeat id "+(pressedSeat.id())
                    +"pressedSeat price "+(pressedSeat.price())
...
//            Log.d("","Test Log clickScheme listofSelectedSeats.size() "+(listofSelectedSeats.size()));
            ...
            listofSelectedSeats.remove(pressedSeat);
    ...
    private void drawTextCentred(Canvas canvas, Paint paint, String text, float cx, float cy){
       try{
//           Log.d("","drawTextCentred canvas values "+(canvas)+" paint value "+(paint)+" text value "+(text)+" cx float "+cx+" cy float "+cy);

//           if(text!=null && textBounds!=null &&cx >0 && cy>0 && textBounds.exactCenterX() >0&& textBounds.exactCenterY()>0 && paint!=null)
           {
//          text=text.trim();
...
//           Log.d("","drawTextCentred Exception values "+(canvas)+" paint value "+(paint)+" text value "+(text)+" cx float "+cx+" cy float "+cy);
...
//        image.setShouldOnMeasureBeCalled(true);
    ...
    public List<Seat> getListOfSelectedSeats() {
        if(listofSelectedSeats!=null && listofSelectedSeats.size()>0){
            return  listofSelectedSeats;
        }
        else{
            return null;
        }
